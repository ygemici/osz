...
int main(int argc, char **argv)
{
    ...
    if(argv[1]==NULL||!strcmp(argv[1],"help")) {
        printf("FS/Z mkfs utility - Copyright 2017 CC-by-nc-sa-4.0 bztsrc@github\n\n"
            "./mkfs (imagefile) (createfromdir)           - create new FS/Z image file of a directory\n"
            "./mkfs (imagefile) initrdrom (romfile)       - create Option ROM from file\n"
            "./mkfs (imagefile) union (path) (members...) - add an union directory to image file\n"
            "./mkfs (imagefile) symlink (path) (target)   - add a symbolic link to image file\n"
            "./mkfs (imagefile) link (path) (target)      - add a hard link to image file\n"
            "./mkfs (imagefile) mime (path) (mimetype)    - change the mime type of a file in image file\n"
            "./mkfs (imagefile) ls (path)                 - parse FS/Z image and list contents\n"
            "./mkfs (imagefile) cat (path)                - parse FS/Z image and return file content\n"
            "./mkfs (imagefile) dump (lsn)                - dump a sector in image into C header format\n"
