 ...
 *
 * Modified for EFI by bztsrc@github
 *
 */
...
extern void CopyMem(void *Dst, const void *Src, unsigned long long len);
...
static void zlib_updatewindow(z_streamp strm, unsigned out)
{
    ...
    if (copy >= state->wsize) {
        CopyMem(state->window, strm->next_out - state->wsize, state->wsize);
        state->write = 0;
        ...
        CopyMem(state->window + state->write, strm->next_out - copy, dist);
        copy -= dist;
        if (copy) {
            CopyMem(state->window, strm->next_out - copy, copy);
            state->write = copy;
            state->whave = state->wsize;
        }
        else {
